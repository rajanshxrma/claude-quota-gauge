...
title_collision_already_present = any(
    h.get("command") and normalize(h["command"]) == normalize(title_collision_hook_command)
    for group in user_prompt_submit
    for h in group.get("hooks", [])
)

if title_collision_already_present:
    print("  title-collision UserPromptSubmit hook already present, left settings.json unchanged")
else:
    user_prompt_submit.append({"hooks": [{"type": "command", "command": title_collision_hook_command, "timeout": 10}]})
    print(f"  added UserPromptSubmit hook: {title_collision_hook_command}")
    print("  (no-ops unless the session-title chip collides with another open session's title)")
